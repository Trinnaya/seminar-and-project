...
Schedule matcher still needs more simple paper

Course Rater

For the Super Planner, two papers from this section point to the same core idea: students understand their academic path better when they can see it visually rather than read it from a handbook. \cite{paper_15} shows that mapping courses as nodes with prerequisite arrows and color coded status helps students track their progress faster and with fewer mistakes, which is exactly how the Super Planner's drag and drop canvas is designed to work. And \cite{paper_13} adds to this by showing that some courses act as bottlenecks because they unlock many others, so knowing which ones those are helps students avoid choices that slow down their graduation. Both of these ideas are built into how the Super Planner connects and displays course bubbles.
