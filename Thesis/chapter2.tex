...
For the technology stack, the MERN-based implementations demonstrated in \cite{paper_11} and \cite{paper_12} confirm that MongoDB, Express.js, React, and Node.js form a proven and cohesive stack for educational web platforms at a similar scope to EGCEye. The REST API design principles outlined in \cite{paper_10} further reinforce that a single API layer can serve multiple interface types while maintaining data consistency — an architecture that EGCEye directly adopts.

For the Schedule Matcher, the Master Busy Map logic \cite{paper_7} provides the core algorithm for deriving common free windows across multiple students' schedules, while TableExtractNet \cite{ngubane2024tableextractnet} informs the image-based table extraction approach used to convert schedule screenshots into structured data.
